...
from utils.constant import LOGO_LIST, today, tomorrow, j2
from Website.models import MatchsAVenir, Data, Iframe
from django.db.models import Q
from rest_framework.permissions import IsAuthenticated
from rest_framework import viewsets
from Website.serializers import IframeSerializer, DataSerializer, MatchsAVenirSerializer


class IframeViewSet(viewsets.ModelViewSet):
    serializer_class = IframeSerializer
    queryset = Iframe.objects.all()
    permission_classes = (IsAuthenticated, )
    filterset_fields = ['championship', 'iframe_stats', 'date_updated']
    search_fields = ["championship"]
...
class DataViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = (IsAuthenticated,)
    filterset_fields = ['championship']
    search_fields = ["championship"]


class MatchsAVenirViewSet(viewsets.ModelViewSet):
    serializer_class = MatchsAVenirSerializer
    queryset = MatchsAVenir.objects.all()
    permission_classes = (IsAuthenticated,)
    filterset_fields = ['match', 'championship', 'date', 'home_team', 'away_team']
    search_fields = ['home_team', 'away_team', 'championship', 'match']
